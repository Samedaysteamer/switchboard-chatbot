...
// -------------------------------------------------------------------
//
// SURGICAL FIXES IN THIS BASELINE:
//  1) Hard syntax errors fixed (restores bot responsiveness).
//  2) ZIP is asked AFTER pricing and BEFORE address (new step collect_zip).
//  3) ZIP validation uses zips.js reliably (tries ./zips.js then ../zips.js).
//  4) Arrival window quick replies reduced to 2 options: "8 to 12", "1 to 5".
//  5) Booking summary does NOT speak a dispatcher phone number (reply here to edit).
...
const BOT_VER    = "2025-10-04-turnlog-v3.1+zipgate-hotfix";
...
function normalizeWindow(input="") {
  ...
  if (t === "8 to 12") return "8 to 12";
  if (t === "1 to 5") return "1 to 5";
  return "";
}
...
// 🔒 robust import: tries ./zips.js then ../zips.js so zip checks never silently disappear
try { validZipCodes = require("./zips.js").validZipCodes || null; } catch {
...
function zipInArea(zip) {
  ...
  if (!VALID_ZIP_SET) return false; // fail-closed (prevents “bad ZIP passes” if list missing)
...
function promptAddress(state) {
  // ✅ ZIP gate (after pricing, before address)
  if (!state.zipVerified) return promptZip(state);
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (correctionReply) {
      if (typeof correctionReply === "string") return respond({ reply: correctionReply, state });
...
Proceed with booking?`;
    ...
    switch (state.step || "choose_service") {
      ...
      case "duct_add_dryer": {
        ...
        const furn = state.duct.add.furnace ? ", +furnace" : "";
        const dry  = state.duct.add.dryer ? ", +dryer vent" : "";
        return respond({ reply: `Your **${state.duct.pkg}** duct cleaning total is **$${total}** (${state.duct.systems} system${state.duct.systems>1?"s":""}${furn}${dry}). Proceed?`, quickReplies: ["Proceed","Change","Skip"], state });
      }
      ...
      case "collect_zip": {
        ...
        if (!z) return respond({ reply: "Please enter a valid **5-digit ZIP code**.", state });

        // fail-closed if list missing (prevents random ZIP passing)
        if (!VALID_ZIP_SET) {
          state.zip = z;
          state.zipVerified = false;
          state.step = "ooa_collect_phone";
          return respond({
            reply: "We’re having trouble verifying service area right now. A team member can confirm.\n\nWhat’s the best **phone number** to reach you?",
            state
          });
        }
      ...
      case "confirm_reuse_address": {
        ...
        return respond({ reply: `What’s the full service address? (street + city + state — ZIP optional)`, state });
      }
      ...
      case "collect_address": {
        if (isQuestion(user)) {
          ...
          if (a) {
            logFAQ(state, user, a);
            return respond({ reply: a + `\n\nWhat’s the full service address? (street + city + state — ZIP optional)`, state, intentHandled:"faq" });
          }
          return respond({ reply: `I can help with that, but first I need your address. What’s the full service address? (street + city + state — ZIP optional)`, state });
        }

        // require street + state; ZIP optional (already collected)
        ...
        if (!hasStreet || !hasState) {
          return respond({ reply: `Please provide your **full service address** (street + city + state). Example: "2314 College St Atlanta GA"`, state });
        }

        // if they include a ZIP in the address, validate it too (fail-closed)
        ...
        // if we still don’t have a verified zip, force zip gate
        if (!state.zipVerified) return respond(promptZip(state));
      ...
      case "ooa_collect_name": {
        ...
        const handoffMsg = `Thanks, ${state.name}! We’ll call ${formatPhone(state.phone)} to confirm if we can service your area.`;
      ...
      case "collect_window": {
        ...
        if (!TIME_WINDOWS.includes(chosen)) return respond({ reply:`Please pick one:`, quickReplies: TIME_WINDOWS, state });
        state.window = chosen;
