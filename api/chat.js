...
// ✅ FIX: ZIP LIST IS IN THE SAME /api FOLDER: ./zips.js
...
/* ========================= ZIP Enforcement Helpers (Fail-Closed) ========================= */
function zipVerdict(zip5) {
...
function buildCleaningBreakdownForZap(state) {
  ...
  if (state.duct) lines.push(`Duct — ${state.duct.pkg} (${state.duct.systems} system${state.duct.systems > 1 ? "s" : ""}) — $${state.duct.total}`);
  return lines.join("\n");
}
...
/* ========================= ZIP Gate (surgical: enforced at promptAddress) ========================= */
function promptAddress(state) {
  // If zip missing, collect zip first (post-price gate)
  ...
  // Fail-closed if zip list missing
  ...
  // ZIP is verified -> proceed to address
...
async function handleCorePOST(req, res) {
  try {
    ...
    if (!user && state.step) return res.status(200).json(repromptForStep(state));
    if (!state.step && user) return res.status(200).json(intro());

    // ZIP collection step (hard enforcement)
    if (state.step === "collect_zip") {
      ...
      // Verified -> move to address
      return res.status(200).json(promptAddress(state));
    }

    // If they somehow jump to address without a verified ZIP, promptAddress will force ZIP first.
    if (state.step === "collect_address") {
      // If zip not verified, this call will re-route to collect_zip / ooa flow
      const gate = promptAddress(state);
      if (gate.step === "collect_zip" || gate.step === "ooa_collect_phone") {
        return res.status(200).json(gate);
      }

      // Allow address capture now
      ...
      // If address contains ZIP, re-validate it (hard safety)
    ...
    // Everything else: keep your existing flow untouched by routing into address gate when needed.
    // If your current logic calls promptAddress(state), ZIP will now be enforced there.
